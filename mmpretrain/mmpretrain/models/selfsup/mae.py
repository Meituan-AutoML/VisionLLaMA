...
from mmpretrain.models import HiViT, VisionTransformer, VisionLLaMA
...
class MAELLaMA(VisionLLaMA):
    ...
    def random_masking(
        ...
        N, L, D = x.shape  # batch, length, dim
        len_keep = int(L * (1 - mask_ratio))
        encoder_cis = self.encoder_cis.unsqueeze(0).repeat(N, 1, 1) # N, L, c/2
        ...
        cis = torch.gather(encoder_cis, dim=1,
                           index=ids_keep.unsqueeze(-1).repeat(1, 1, embed_dim // num_heads // 2))
        ...
        return x_masked, mask, ids_restore, cis
    ...
    def forward(
        ...
        if self.encoder_cis is None:
            embed_dim = self.embed_dims
            num_heads = self.arch_settings['num_heads']
            from mmpretrain.models import precompute_freqs_cis_2d
            self.encoder_cis = precompute_freqs_cis_2d(embed_dim // num_heads, self.num_patches).to(x.device)
        ...
        if mask is None or False:
            ...
            # masking: length -> length * mask_ratio
            x, mask, ids_restore, cis = self.random_masking(x, self.mask_ratio)
            ...
            x = torch.cat((self.cls_token.expand(B, -1, -1), x), dim=1) # add cls token to make it work with decoder or we have to change the imp of decoder.
            # Use final norm
            x = self.norm1(x)
