...
def apply_rotary_emb(
    ...
    xq_ = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2)) # B, N, H, Hc -> B N H Hc/2
    xk_ = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2))
    freqs_cis = reshape_for_broadcast(freqs_cis, xq_) # B, N, H, Hc/2
    xq_out = torch.view_as_real(xq_ * freqs_cis).flatten(3) # B, N, H, Hc/2 -> B, N, H, Hc/2, 2 >B, N, H, Hc
...
class MultiheadAttentionRoPE(BaseModule):
    """Multi-head Attention based on 2D RoPE Module.
    ...
    def __init__(self,
                 ...
                 layer_scale_init_value=0.,
                 init_cfg=None):
        super(MultiheadAttentionRoPE, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, x, freqs_cis):
        ...
        q, k = apply_rotary_emb(q, k, freqs_cis=freqs_cis) # B, N, H, Hc
