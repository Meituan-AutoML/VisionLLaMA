...
from ..utils import (MultiheadAttention, MultiheadAttentionRoPE, SwiGLUFFNFused, LLaMAMLP, build_norm_layer,
                     resize_pos_embed, to_2tuple)
from .base_backbone import BaseBackbone

def precompute_freqs_cis_2d(dim: int, end: int, theta: float = 10000.0, scale=1.0):
    H = int( end**0.5 )
    # assert  H * H == end
    flat_patch_pos = torch.arange(end) # N = end
...
class TransformerEncoderLayerRoPE(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(TransformerEncoderLayerRoPE, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        super(TransformerEncoderLayerRoPE, self).init_weights()
...
class VisionLLaMA(BaseBackbone):
    """Vision Transformer.

    A PyTorch implement of : `Vision LLaMA`_
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(VisionLLaMA, self).__init__(init_cfg)
        ...
        for i in range(self.num_layers):
            ...
            self.layers.append(TransformerEncoderLayerRoPE(**_layer_cfg))
    ...
    def init_weights(self):
        super(VisionLLaMA, self).init_weights()
    ...
    def forward(self, x):
        B = x.shape[0]
        if self.encoder_cis is None:
            embed_dim = self.embed_dims
            num_heads = self.arch_settings['num_heads']
            num_patches = self.patch_resolution[0] * self.patch_resolution[1]

            self.encoder_cis = precompute_freqs_cis_2d(embed_dim // num_heads, num_patches).to(x.device)
        ...
        #     num_extra_tokens=self.num_extra_tokens) we non-longer need pos_embed
