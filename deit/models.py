...
@register_model
def deit_tiny_patch16_160(pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=160, patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
...
@register_model
def deit_base_patch16_160(pretrained=False, **kwargs):
    model = VisionTransformer(
